...
| `squash --to-main`: promotes a task branch onto `main` via real merge, originals untouched, **and** the worktree guard that refuses `--to-main` from inside a linked worktree | **—** | no e2e, no cmd-level unit test of any kind (only the underlying `service.MergeService`/`BranchService` calls are unit-tested indirectly). This is a real, security-relevant gap: the worktree guard (squash.go) has zero coverage at any level |
...
| `sandbox exec`, `shell`, `grants`/`grant`, `list`, `status`, `published`, `remove`, `image install` | unit only | cmd-level unit tests exist for each (sandbox_test.go, sandbox_grant_test.go, sandbox_shell_test.go, sandbox_image_test.go); **none of these seven subcommands is invoked in any e2e script**, and the previous matrix's generic "lifecycle verbs (exec, shell, grants, image)" row didn't even name `list`/`status`/`published`/`remove`/`image install` individually. `sandbox published` (SEC-09 one-way port publishing) is the most notable of these — a security-relevant, dedicated subcommand with no e2e proof and no prior mention in this file at all |
| `mgit sandbox claude-hook` (hidden; the PreToolUse hook a real Claude Code harness drives, MGIT-11.11.1) | unit only | claude_hook_test.go (7 cases: healthy/down/no-sandbox/deny-rule/non-bash/malformed-stdin/empty-cwd) — thorough at the unit level, but the actual JSON-stdin/stdout contract has **zero real-process proof** and was not mentioned anywhere in the previous matrix despite being a security-relevant, named feature |
...
| `mgit worktree prune` | **—** | service-layer unit tests exist (`service_operations_test.go`); no cmd-level test, no e2e. Not even named in the previous matrix's "worktree add/list/remove" row despite being a real fourth subcommand |
