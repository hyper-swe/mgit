...
|---------|---------|-------------|----------------------|---------------|
| `github.com/firecracker-microvm/firecracker-go-sdk` | Linux KVM microVM control (Firecracker-class VMM) | 1.0.0 | linux only; pure Go (VMM binary is COTS, assessed per FR-17.30/31) | **Linux backend (FR-17.15).** Battle-tested lineage (AWS Lambda, E2B) for running untrusted code. Rejected alternatives at the time: libkrun (CGO in control path, immature Go bindings) — **this rejection was revisited and reversed for an ADDITIONAL, opt-in backend; see the `libkrun` row below and ADR-010**; hand-rolled Cloud Hypervisor client (~1,500 lines), QEMU/libvirt (surface + boot latency vs NFR-17.2). |
...
| `libkrun` + `libkrunfw` (C libraries, via cgo) | Cross-platform microVM VMM (KVM on Linux, HVF on macOS) with a bundled guest kernel | libkrun 1.19.4 / libkrunfw 5.5.0 | linux+darwin arm64; **CGO — confined to mgit-sandboxd**; the macOS build links it by default and the **Linux release build** links it (`-tags libkrun`; a plain Linux `go build` stays CGO-free firecracker); **macOS: bundled and patched — the release archive ships a libkrun v1.19.4 built from source with the mgit-carried patch `libkrun-mgit-225.patch` (fixes MGIT-225) applied, beside mgit-sandboxd in `lib/`, with the source tag and the patch's sha256 pinned in `scripts/sandbox-image/pins.env` (`LIBKRUN_VERSION`, `LIBKRUN_DARWIN_PATCH`, `LIBKRUN_DARWIN_PATCH_SHA256`); libkrunfw stays a user-installed Homebrew prerequisite. Linux: bundled in the release archives beside mgit-sandboxd, with the corresponding kernel source published as release assets (ADR-016)** | **Additional cross-platform backend (FR-17.15, ADR-010).** One VMM seam across Linux/KVM and macOS/HVF that brings its own kernel, retiring mgit's own kernel build and guest-image distribution. Maintained by the containers org (Red Hat/IBM) and load-bearing under `podman machine`. Reverses the earlier rejection above on the strength of the MGIT-61.5 spike (API fit, real boot, provable default-deny egress, clean churn). **Licences:** libkrun **Apache-2.0**; libkrunfw **GPL-2.0-only + LGPL-2.1-only** — the GPL applies to the guest kernel it bundles, which runs as a SEPARATE program inside the VM and imposes no copyleft on mgit. Guardrails: link libkrunfw **dynamically** and keep mgit Apache-2.0. On macOS libkrunfw stays a user-installed prerequisite, and the macOS release **redistributes** a patched libkrun v1.19.4 (one mgit-carried patch, `libkrun-mgit-225.patch`, fixes MGIT-225; its sha256 pinned in `scripts/sandbox-image/pins.env`), so the darwin archive ships libkrun's license texts (`THIRD_PARTY/`) and states the applied patch and its digest in `THIRD_PARTY/SOURCES.txt`. On Linux the release **redistributes** libkrun and libkrunfw (no distribution packages either one), so every such release ships their license texts in the archive (`THIRD_PARTY/`) and publishes the corresponding source of the GPL-2.0 kernel inside libkrunfw as release assets (ADR-016, which reverses the earlier "never redistribute" stance for Linux only). busybox is still never redistributed. Does not replace firecracker/vzf; both remain first-class. |
...
| `gvisor.dev/gvisor` | gVisor userspace TCP/IP stack (netstack) for the libkrun host network gateway | 0.0.0-20260224225140 | sandbox-confined; pure Go; pinned by RELEASE TAG (upstream publishes no semver) | **Rootless egress enforcement (FR-17.7, FR-17.8, SEC-04, ADR-010).** libkrun's virtio-net backing is a unixgram socket carrying ethernet frames, which the existing L4 CONNECT proxy cannot serve. Embedding netstack terminates the guest's TCP in-process and exposes `tcp.NewForwarder`, a per-connection hook where `egress.Authorizer` decides allow/RST — so the POLICY is unchanged and only the data path is new. CAN replace the tap+`iptables` engine — which is Linux-only and REQUIRES ROOT — with one portable implementation (Linux, macOS, future Windows) that mutates no host firewall state and makes allowlist enforcement unit-testable without a VM; today it backs the libkrun backend ONLY, and firecracker/vzf still use the tap engine. TCP only at present (no UDP/DNS forwarder yet). Apache-2.0, maintained by Google. **Pin policy:** track the version Tailscale ships (the largest production netstack consumer) — see docs/release/RELEASE-CHECKLIST.md; NEVER pin `@latest` (gvisor tip has shipped a package-name conflict that breaks module consumers). Rejected: `github.com/noisysockets/netstack` (semver fork, but newest release 2024-09-20 — ~2 years of missed netstack security fixes on a containment boundary); shelling out to `gvproxy`/`passt` (no deny-by-default hook, and reintroduces an extra binary to ship/sign/install). |
...
| 2026-07-30 | Bumped go-git/v5 5.17.2 → 5.19.1 and go-billy/v5 5.8.0 → 5.9.0; Go toolchain 1.26.4 → 1.26.5 | govulncheck found 7 vulnerabilities in called code (go-git: GO-2026-5693 malformed-object panic/resource-exhaustion, GO-2026-5496 SSH transport escaping, GO-2026-5074 improper tree-entry parsing; go-billy: GO-2026-5597 path traversal, GO-2026-5490 symlink-cycle DoS; go-git via test code only: GO-2026-5105 credential leak via redirect; stdlib crypto/tls: GO-2026-5856). Both floors (5.13.0 / 5.8.0) are unchanged — this stays inside the already-approved v5 line, v6 remains rejected. Re-run: 0 vulnerabilities in called code. The upgrade also surfaced and fixed a real defect in mgit's own tree-flattening code (internal/store/git/plumbing.go): it silently discarded ANY tree-walker error as end-of-iteration, which the GO-2026-5074 fix's new per-entry path validation would have turned into a silently-empty target tree (and, worse, a spurious mass deletion via materializeCommit's absent-from-target cleanup pass) instead of an aborted checkout. Full suite + `-race` + live firecracker e2e re-verified after both changes. |
