...
| `libkrun` + `libkrunfw` (C libraries, via cgo) | Cross-platform microVM VMM (KVM on Linux, HVF on macOS) with a bundled guest kernel | libkrun 1.19.4 / libkrunfw 5.5.0 | linux+darwin arm64; **CGO — confined to mgit-sandboxd**; **opt-in at build time via `-tags libkrun`**; user-installed prerequisite, NOT redistributed by mgit | **Additional cross-platform backend (FR-17.15, ADR-010).** One VMM seam across Linux/KVM and macOS/HVF that brings its own kernel, retiring mgit's own kernel build and guest-image distribution. Maintained by the containers org (Red Hat/IBM) and load-bearing under `podman machine`. Reverses the earlier rejection above on the strength of the MGIT-61.5 spike (API fit, real boot, provable default-deny egress, clean churn). **Licences:** libkrun **Apache-2.0**; libkrunfw **GPL-2.0-only + LGPL-2.1-only** — the GPL applies to the guest kernel it bundles, which runs as a SEPARATE program inside the VM and imposes no copyleft on mgit. Guardrails: link libkrunfw **dynamically**, ship the Apache NOTICE + LGPL text, and do **not** redistribute libkrunfw/busybox binaries (treated as a user-installed prerequisite, exactly like the `firecracker` binary today), keeping mgit Apache-2.0. Does not replace firecracker/vzf; both remain first-class. |
