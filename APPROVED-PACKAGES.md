...
| `libkrun` + `libkrunfw` (C libraries, via cgo) | Cross-platform microVM VMM (KVM on Linux, HVF on macOS) with a bundled guest kernel | libkrun 1.19.4 / libkrunfw 5.5.0 | linux+darwin arm64; **CGO — confined to mgit-sandboxd**; the macOS build links it by default and the **Linux release build** links it (`-tags libkrun`; a plain Linux `go build` stays CGO-free firecracker); **macOS: user-installed prerequisite (Homebrew), not redistributed. Linux: bundled in the release archives beside mgit-sandboxd, with the corresponding kernel source published as release assets (ADR-016)** | **Additional cross-platform backend (FR-17.15, ADR-010).** One VMM seam across Linux/KVM and macOS/HVF that brings its own kernel, retiring mgit's own kernel build and guest-image distribution. Maintained by the containers org (Red Hat/IBM) and load-bearing under `podman machine`. Reverses the earlier rejection above on the strength of the MGIT-61.5 spike (API fit, real boot, provable default-deny egress, clean churn). **Licences:** libkrun **Apache-2.0**; libkrunfw **GPL-2.0-only + LGPL-2.1-only** — the GPL applies to the guest kernel it bundles, which runs as a SEPARATE program inside the VM and imposes no copyleft on mgit. Guardrails: link libkrunfw **dynamically** and keep mgit Apache-2.0. On macOS libkrunfw stays a user-installed prerequisite. On Linux the release **redistributes** libkrun and libkrunfw (no distribution packages either one), so every such release ships their license texts in the archive (`THIRD_PARTY/`) and publishes the corresponding source of the GPL-2.0 kernel inside libkrunfw as release assets (ADR-016, which reverses the earlier "never redistribute" stance for Linux only). busybox is still never redistributed. Does not replace firecracker/vzf; both remain first-class. |
