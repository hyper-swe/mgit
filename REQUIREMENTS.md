...
In every mode that attaches a working NIC (`allowlist`, `open`) the host MUST deliver the guest's own network configuration — address, prefix, default gateway and resolver — and the guest MUST apply it at boot before serving any command. The guest supervisor is PID 1, so no other software in the sandbox will do it. A guest left unaddressed fails every flow, allowed or denied, which is **indistinguishable from correctly enforced containment** unless a test asserts that an ALLOWED flow succeeds — therefore every deny assertion MUST be paired with a matching allow assertion, and a denial MUST be distinguishable by reason (a policy reset) from an absent network. Refs: MGIT-68.
...
**FR-17.11.1** Exec-stream liveness. A client blocked on an exec stream MUST be able to distinguish a guest that is working from a daemon that stopped answering, and MUST report the second as such. The daemon MUST emit a periodic liveness signal while an exec is outstanding, and the client MUST treat the GAP between signals — never the command's total duration — as its deadline: a command that produces no output for any length of time MUST NOT be killed for it (MGIT-122). The liveness signal MUST be gated on evidence the daemon can still answer for its own state, so that a daemon which is scheduled but wedged stops signalling rather than certifying a liveness it does not have. A stall MUST be reported as a DAEMON failure — never as a guest failure and never with an in-guest memory diagnosis (MGIT-118). The daemon MUST emit its FIRST liveness signal before any per-command work begins, so that the client's first gap is judgeable. A peer that reaches the exec stream has passed the control-plane version handshake (IDD §8) and has thereby stated that it speaks this signal, so silence exceeding the gap deadline MUST be reported as a daemon stall and MUST NOT be excused as an older build. (This supersedes MGIT-133's rule that a daemon emitting no signal at all be treated as merely older: a build too old to signal is also too old to handshake, so it is refused before the exec stream and never served — MGIT-138.) `mgit sandbox exec` and `mgit run` MUST accept a `--timeout` populating `ExecRequest.Timeout`, defaulting to unbounded. Refs: MGIT-138, MGIT-136, MGIT-133, MGIT-122, MGIT-118; IDD §7, §8.
