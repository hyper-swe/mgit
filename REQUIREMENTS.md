...
**FR-17.11.1** Exec-stream liveness. A client blocked on an exec stream MUST be able to distinguish a guest that is working from a daemon that stopped answering, and MUST report the second as such. The daemon MUST emit a periodic liveness signal while an exec is outstanding, and the client MUST treat the GAP between signals — never the command's total duration — as its deadline: a command that produces no output for any length of time MUST NOT be killed for it (MGIT-122). The liveness signal MUST be gated on evidence the daemon can still answer for its own state, so that a daemon which is scheduled but wedged stops signalling rather than certifying a liveness it does not have. A stall MUST be reported as a DAEMON failure — never as a guest failure and never with an in-guest memory diagnosis (MGIT-118). A daemon that emits no liveness signal at all MUST be treated as offering none (an older daemon), not as stalled. `mgit sandbox exec` and `mgit run` MUST accept a `--timeout` populating `ExecRequest.Timeout`, defaulting to unbounded. Refs: MGIT-133, MGIT-122, MGIT-118; IDD §7.
