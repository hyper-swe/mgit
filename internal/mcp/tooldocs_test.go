...
func TestToolDocs_CoversRegisteredSurface(t *testing.T) {
	...
	want := map[string][]string{
		"mgit_commit":          {"agent_id", "allow_empty", "allow_large", "message", "stage_all", "task_id"},
		...
		"mgit_sandbox_policy":  {"action", "allow", "drain", "task_id"},
		"mgit_sandbox_export":  {"guest_path", "host_path", "task_id"},
	}
