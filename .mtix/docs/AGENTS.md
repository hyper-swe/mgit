...
> **Start a task with `mgit work`.** `mgit work <path> --task <ID>` provisions a task-bound mgit worktree. When containment is requested (`--sandbox`) or active it wires the agent shell to route through `mgit run` (the sandbox airlock); with no sandbox on the machine it installs no routing and you run commands normally on the host (the worktree's `CLAUDE.md` states the posture, and `mgit work` prints a `Containment: …` line). Inside the worktree, `mgit commit -m "..."` records each step (task ID auto-inherited), `mgit log`/`mgit status`/`mgit diff` orient you, `mgit rollback`/`mgit cherry-pick` course-correct without restarting, and a new line forks as a new task worktree from the project root (`mgit work <new-path> --task-id <new-task-id> --base <good-commit>`; a task worktree refuses `mgit checkout -b`), and `mgit squash --task-id <ID> --to-git` collapses the work for the land. See [MGIT_WORKING_DISCIPLINE.md](MGIT_WORKING_DISCIPLINE.md).
